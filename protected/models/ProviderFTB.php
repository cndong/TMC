    ...
    private function _initFlightList($data) {
        ...
        foreach ($flightRoutes as $routeIndex => $flightRoute) {
            ...
            foreach ($flightRoute['segments']['at_n_search_segment_v_o'] as $flightSegment) {
                ...
                $standardPrice = max($flightSegment['basic_cabin_price'], $flightSegment['ticket_price'], $flightSegment['price']);
                $flights[$flightKey]['cabins'][$flightSegment['cabin']] = array(
                    ...
                    'discount' => floatval(sprintf('%.1f', $flightSegment['ticket_price'] * 10 / $standardPrice)),
                    'adultPrice' => min($flightSegment['ticket_price'], $flightSegment['price']),
                    'childPrice' => round($standardPrice * DictFlight::RATE_CHILD / 1000) * 1000,
                    'babyPrice' => round($standardPrice * DictFlight::RATE_BABY / 1000) * 1000,
