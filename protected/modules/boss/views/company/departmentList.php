        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                <form class="form-inline" action="<?php echo $this->createUrl('departmentList'); ?>">
                    <span>部门列表</span>
                    <select name="companyID" class="form-control input-sm">
                        <option value="0">----请选择企业----
                        <?php 
                            foreach ($companies as $company) {
                                $selected = $company->id == $params['companyID'] ? ' selected' : '';
                                echo "<option value='{$company->id}'{$selected}>{$company->name}";
                            }
                        ?>
                    </select>
